...
@ApplyOptions<Listener.Options>((ctx) => ({ event: "trackStart", emitter: ctx.store.container.client.manager }))
export default class extends Listener {
	public async run({ player, track }: { player: Player; track: LoadedTrack }): Promise<void> {
		...
		if (old && this.client.config.get(player.guild)?.deleteAnnounce) await channel.messages.delete(old);
				...
				.send({
					embeds: [
						this.client.utils
							...
							.setTitle(
								this.translate.translate(player.guild, "MusicGeneral:announce.title", {
									title: track.title
								})
							)
							.setDescription(
								this.translate.translate(player.guild, "MusicGeneral:announce.description", {
