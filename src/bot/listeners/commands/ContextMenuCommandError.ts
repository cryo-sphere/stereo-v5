...
import { ContextMenuInteraction, DiscordAPIError, HTTPError } from "discord.js";
...
export default class extends Listener {
	public async run(error: any, { interaction, command }: ContextMenuCommandErrorPayload) {
	...
	private reply(interaction: ContextMenuInteraction, str: string) {
	...
	private isSilencedError(interaction: ContextMenuInteraction, error: DiscordAPIError | HTTPError) {
		return ignoredCodes.includes(error.code) || this.isDirectMessageReplyAfterBlock(interaction, error);
	}

	private isDirectMessageReplyAfterBlock(interaction: ContextMenuInteraction, error: DiscordAPIError | HTTPError) {
	...
	private generateUnexpectedErrorMessage(interaction: ContextMenuInteraction, error: Error) {
	...
	private getWarnError(interaction: ContextMenuInteraction) {
