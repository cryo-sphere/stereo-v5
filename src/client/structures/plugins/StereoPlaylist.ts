...
export class StereoPlaylist extends Plugin {
	...
	private readonly regex =
		/(?:https:\/\/stereo-bot\.tk\/|stereo:)(playlists)?[\/:]([A-Za-z0-9|\-|_]+)/;

	private readonly SpotifyRegex =
		/(?:https:\/\/open\.spotify\.com\/|spotify:)(?:.+)?(track|playlist|album)[\/:]([A-Za-z0-9]+)/gi;
	private readonly DeezerRegex =
		/(?:https:\/\/www\.deezer\.com\/|deezer:)(?:[A-Za-z]+)(?:.+)?(track|playlist|album)[\/:]([A-Za-z0-9]+)/gi;
	...
	private async getPlaylist(id: string, requester: string) {
		const playlist = await this.get<{ playlist: Playlist; isOwner: boolean }>(
