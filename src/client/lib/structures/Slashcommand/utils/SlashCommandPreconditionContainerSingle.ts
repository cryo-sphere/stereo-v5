...
export interface SlashCommandPreconditionSingleResolvableDetails<K extends SlashCommandPreconditionKeys = SlashCommandPreconditionKeys> {
	name: K;
...
export class SlashCommandPreconditionContainerSingle implements ISlashCommandPreconditionContainer {
	...
	public run(
		...
		const precondition = container.stores.get("slashCommandPreconditions").get(this.name);
		if (precondition) return precondition.run(interaction, command, { ...context, ...this.context });
