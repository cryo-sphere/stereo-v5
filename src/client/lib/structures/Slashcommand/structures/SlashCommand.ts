...
import { SlashCommandPreconditionContainerArray, SlashCommandPreconditionEntryResolvable } from "../utils/SlashCommandPreconditionContainerArray";
import type {
	CommandInteraction,
...
export abstract class SlashCommand<T = Omit<CommandInteractionOptionResolver<CacheType>, "getMessage" | "getFocused">> extends AliasPiece {
	...
	private resolveConstructorPreConditionsRunType(
		...
		if (runIn.length === 0) throw new Error(`${this.constructor.name}[${this.name}]: "runIn" was specified as an empty array.`);
		...
		if (guild) {
			...
			if (guildText) {
				...
				if (guildPublicThread) preconditions.append(SlashCommandPreConditions.GuildPublicThreadOnly);

				if (guildPrivateThread) preconditions.append(SlashCommandPreConditions.GuildPrivateThreadOnly);
			}
...
export interface SlashCommandOptions extends PieceOptions {
	...
	requiredClientPermissions?: PermissionString[];
...
export type CommandArg = ApplicationCommandOptionData[];
